...
public class AssetView {
    ...
    public VBox getView() {
        ...
        HBox filterBar = new HBox(10, filterLabel,
        ...
        importBtn.setOnAction(e -> {
            CsvImporter.importCsv("Import Assets CSV", true, fields -> {
                ...
                String body = "{" +
                        ...
                        "\"departmentId\":" + (fields[6].isEmpty()
                        ? 0 : Integer.parseInt(fields[6])) + "," +
                        "\"location\":\"" + fields[7] + "\"," +
                        "\"status\":\"" + fields[8] + "\"," +
                        "\"assignedTo\":0," +
                        "\"assignedDate\":null," +
                        ...
                        "\"warrantyExpiry\":null," +
                        "\"vendorId\":0," +
                        "\"purchaseCost\":" + (fields[9].isEmpty()
                        ? 0.0 : Double.parseDouble(fields[9])) + "," +
                        "\"notes\":\"" + fields[10] + "\"" +
                        "}";
    ...
    private void showAddDialog() {
        ...
        grid.add(new Label("Asset Tag *:"), 0, 0);
        grid.add(assetTagField, 1, 0);
        ...
        grid.add(new Label("Serial No:"),   0, 5);
        ...
        grid.add(new Label("Cost:"),        0, 9);
        grid.add(costField, 1, 9);
        ...
        assetTagField.textProperty()
                .addListener((o, ov, nv) -> check.run());
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            try {
                int deptId = deptField.getText().trim().isEmpty()
                        ? 0 : Integer.parseInt(deptField.getText().trim());
                double cost = costField.getText().trim().isEmpty()
                        ? 0.0 : Double.parseDouble(
                        costField.getText().trim());
                if (saveAsset(assetTagField.getText(),
                        nameField.getText(), categoryBox.getValue(),
                        brandField.getText(), modelField.getText(),
                        serialField.getText(), deptId,
                        locationField.getText(), statusBox.getValue(),
    ...
    private String validateAsset(String tag, String name,
                                 String brand, String cost,
                                 String deptId) {
        ...
        if (!deptId.trim().isEmpty()) {
            try { Integer.parseInt(deptId.trim()); }
                ...
                return "Dept ID must be a number.";
            }
        }
        return null;
    }

    private void showEditStatusDialog(Asset asset,
